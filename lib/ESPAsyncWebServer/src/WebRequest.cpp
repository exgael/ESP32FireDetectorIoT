...
void AsyncWebServerRequest::_onError(int8_t error) { (void)error; }
...
void AsyncWebServerRequest::onDisconnect(ArDisconnectHandler fn) { _onDisconnectfn = fn; }
...
void AsyncWebServerRequest::_addParam(AsyncWebParameter* p) { _params.add(p); }

void AsyncWebServerRequest::_addPathParam(const char* p) { _pathParams.add(new String(p)); }
...
void AsyncWebServerRequest::_handleUploadByte(uint8_t data, bool last) {
    ...
    if (last || _itemBufferIndex == 1460) {
        ...
        if (_handler)
            _handler->handleUpload(this, _itemFilename, _itemSize - _itemBufferIndex, _itemBuffer, _itemBufferIndex,
                                   false);
        _itemBufferIndex = 0;
    }
}
...
#define itemWriteByte(b)                \
    ...
    if (_multiParseState == WAIT_FOR_RETURN1) {
        ...
        if (_boundaryPosition < _boundary.length() && _boundary.c_str()[_boundaryPosition] != data) {
            ...
            if (!_itemIsFile) {
                ...
                if (_itemSize) {
                    // check if authenticated before calling the upload
                    if (_handler)
                        _handler->handleUpload(this, _itemFilename, _itemSize - _itemBufferIndex, _itemBuffer,
        ...
        if (data == '-' && (_contentLength - _parsedLength - 4) != 0) {
            ...
            // we can have more info on the matter!\n", _contentLength - _parsedLength - 4);
...
size_t AsyncWebServerRequest::headers() const { return _headers.length(); }
...
size_t AsyncWebServerRequest::params() const { return _params.length(); }
...
AsyncWebServerResponse* AsyncWebServerRequest::beginResponse(File content, const String& path,
    ...
    if (content == true) return new AsyncFileResponse(content, path, contentType, download, callback);
    return NULL;
}
...
bool AsyncWebServerRequest::authenticate(const char* username, const char* password, const char* realm,
    ...
    if (_authorization.length()) {
        if (_isDigest)
            return checkDigestAuthentication(_authorization.c_str(), methodToString(), username, password, realm,
                                             passwordIsHash, NULL, NULL, NULL);
...
bool AsyncWebServerRequest::authenticate(const char* hash) {
    if (!_authorization.length() || hash == NULL) return false;
...
const String& AsyncWebServerRequest::arg(size_t i) const { return getParam(i)->value(); }

const String& AsyncWebServerRequest::argName(size_t i) const { return getParam(i)->name(); }

...
bool AsyncStaticWebHandler::canHandle(AsyncWebServerRequest* request) {
    ...
    if (_getFile(request)) {
        ...
        if (_last_modified.length()) request->addInterestingHeader("If-Modified-Since");

        if (_cache_control.length()) request->addInterestingHeader("If-None-Match");
...
bool AsyncStaticWebHandler::_getFile(AsyncWebServerRequest* request) {
    ...
    // We can skip the file check and look for default if request is to the root of a directory or that request path
    // ends with '/'
    ...
    if (request->_tempFile == true) {
        ...
        if (_last_modified.length() && _last_modified == request->header("If-Modified-Since")) {
            ...
            AsyncWebServerResponse* response =
                new AsyncFileResponse(request->_tempFile, filename, String(), false, _callback);

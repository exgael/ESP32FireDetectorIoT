...
void wifi_printstatus(int C)
{
    ...
    if (C) {
        ...
        USE_SERIAL.printf(
            "	Received Signal Strength Indication : %ld dBm", WiFi.RSSI());
        USE_SERIAL.printf(
            "	Received Signal Strength Indication : %ld %",
...
void wifi_connect_basic(String hostname, String ssid, String passwd)
{
    ...
    WiFi.disconnect(
        true); // Disconnect from an AP if it was previously connected
...
#define MinimumWiFiRSSI -90
    ...
    if (N > 0) { // Print descriptions if some ?
        ...
        for (int i = 0; i < N; i++) {
            USE_SERIAL.printf(
                "#%d => Signal Strength (higher is better): %ld dBm",
                i,
                WiFi.RSSI(i));

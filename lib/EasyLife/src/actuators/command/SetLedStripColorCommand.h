   ...
   public:
    SetLedStripColorCommand(
        ...
        : controller(controller),
          index(index),
          color(color),
          logger("SetLedStripColorCommand")
    {
    ...
    void execute() override
    {
        try {
            ...
            logger.error(
                "Failed to set LED color at index %d: %s", index, e.what());
        } catch (...) {
            logger.error(
                "Unknown error occurred while setting LED color at index %d.",
                index);
        }
    }

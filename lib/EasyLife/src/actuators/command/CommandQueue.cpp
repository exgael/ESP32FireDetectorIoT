...
void CommandQueue::processCommands() noexcept {
    try {
        while (!commandQueue.empty()) {
            ...
            if (command) {
                try {
                    ...
                    logger.error("Command execution failed: %s", std::string(e.what()));
                } catch (...) {
                    logger.error("Unknown error occurred during command execution.");
                }
                delete command;
            } else {
        ...
        logger.error("Error in processing command queue: %s", std::string(e.what()));
    } catch (...) {

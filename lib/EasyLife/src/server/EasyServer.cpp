...
EasyServer::EasyServer(int port) : server(port), port(port), logger("Server")
{
}
...
void EasyServer::use(MiddlewareFunction middleware)
{
    ...
    Handler handler)
{
    server.on(
        path.c_str(),
        method,
        [this, routeMiddleware, handler](AsyncWebServerRequest *rawRequest) {
            Request req(rawRequest);
            Response res(rawRequest);

            // Combine global and route middleware
            MiddlewareChain allMiddleware =
                buildRouteMiddlewareChain(routeMiddleware);
            ...
            NextFunction next = [&]() {
                if (currentIndex < allMiddleware.size()) {
                    MiddlewareFunction currentMiddleware =
                        allMiddleware[currentIndex];
                    currentIndex++;
...
MiddlewareChain EasyServer::buildRouteMiddlewareChain(
    const MiddlewareChain &routeMiddleware)
{
    MiddlewareChain allMiddleware;

    // Add globals
    allMiddleware.insert(
        allMiddleware.end(), globalMiddleware.begin(), globalMiddleware.end());

    // Add route specific
    allMiddleware.insert(
        allMiddleware.end(), routeMiddleware.begin(), routeMiddleware.end());

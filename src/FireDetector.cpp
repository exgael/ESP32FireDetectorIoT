...
const bool FireDetector::checkIfFireDetected() const noexcept
{
    ...
    int highLuminosityCount =
        countExceedances(luminosityBuffer, luminosityThreshold);
    int highTemperatureCount =
        countExceedances(temperatureBuffer, temperatureThreshold);

    bool luminosityAlert =
        checkAlertCondition(highLuminosityCount, luminosityBuffer.size());
    bool temperatureAlert =
        checkAlertCondition(highTemperatureCount, temperatureBuffer.size());
...
void FireDetector::updateTemperatureThreshold(int new_threshold) noexcept
{
    temperatureThreshold = new_threshold;
}

void FireDetector::updateLuminosityThreshold(int new_threshold) noexcept
{
...
bool FireDetector::checkAlertCondition(
    int exceedCount,
    int buffer_size,
    float thresholdPercentage) const noexcept
{

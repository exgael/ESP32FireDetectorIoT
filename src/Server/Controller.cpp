...
Handler rootControllerHandler(
    ...
    return [&](Request& req, Response& res) {
        std::map<String, std::function<String()>> placeholderMap = {
            {"TEMPERATURE", [&]() { return String(sensorManager.getTemperature()); }},
            {"LIGHT", [&]() { return String(sensorManager.getLuminosity()); }},
...
Handler getValuesControllerHandler(
    ...
    return [&](Request& req, Response& res) {
        ...
        ParamHandler paramHandlers[] = {
            {"temperature", [&](JsonDocument& json) { json["temperature"] = sensorManager.getTemperature(); }},
            {"light", [&](JsonDocument& json) { json["light"] = sensorManager.getLuminosity(); }},
         ...
         * 
         * It receives the parameter name, and uses `paramHandlers` to find the correct handler.
         */
...
Handler setReportControllerHandler(
    Reporter& reporter,
    Logger& logger
) noexcept {
    return [&](Request& req, Response& res) {
        ...
        logger.info("Target set to IP: %s, Port: %d, Sampling Period: %d", 
                    target_ip.c_str(), target_port_int, target_sp_int);
